...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { icons, screenshots, revert, key } = body;

    // Authorize either as a logged-in admin OR via the shared DevTools key
    // (so it works from a public/preview session with no admin login).
    if (key !== DEV_KEY) {
