...
Deno.serve(async (req) => {
  try {
    ...
    // Find all lines with "HEBREWS"
    const hebrewsLines = [];
    for (let i = 0; i < lines.length; i++) {
      const trimmed = lines[i].trim();
      if (trimmed.toUpperCase().includes('HEBREW')) {
        hebrewsLines.push(`${i}: ${trimmed}`);
        if (hebrewsLines.length >= 20) break;
      }
    }
    
    return Response.json({ hebrewsLines });
