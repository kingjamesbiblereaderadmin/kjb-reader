  ...
  try {
    ...
    if (tab === 'gospel') {
      ...
      for (let bi = 0; bi < BOOK_ORDER.length; bi++) {
        ...
        if (maxCh > 1) {
          let chapLinks = '<p class="fb-chaplinks">';
        ...
        for (let ch = 1; ch <= maxCh; ch++) {
          ...
          body += '<h3 class="fb-chap"><a name="b' + bi + 'c' + ch + '" id="b' + bi + 'c' + ch + '"></a>Chapter ' + ch + ' <a href="#b' + bi + '" class="fb-chaptop">&uarr; book top</a></h3>';
    ...
    const html = '<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>KJB Reader (Legacy)</title><style>' + STYLE + (isDark ? DARK_STYLE : '') + '</style></head><body><div class="hdr"><h1>KJB Reader (Legacy)</h1><p>King James Bible &mdash; Pure Cambridge Edition</p></div>' + banner + '<div class="tabs">' + tabsHtml + '</div><div class="wrap" id="wrap">' + bodyInner + '</div>' + ENHANCE_SCRIPT + '</body></html>';
