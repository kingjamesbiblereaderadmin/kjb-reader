...
Deno.serve(async (req) => {
  try {
    ...
    if (tab === 'gospel') {
      ...
      // correctly even when reached via the /legacy React redirect (which
      // would otherwise strip the hash). Links become "<fullUrl>#bN".
      const fbBase = esc(basePath) + '?tab=fullbible' + idSuffix;
