...
// Emulate frontend parsePceText logic
const RTF_TITLE_MAP = {
...
Deno.serve(async (req) => {
  let report = "--- FRONTEND ALGORITHM SYNC ANALYSIS ---\n";
  try {
    ...
    report += `Frontend Modulo output for 20260606: ${bk} ${ch}:${vs}\n\n`;
    report += `Total books parsed: ${bookNames.length}\n`;
    report += `Books parsed: ${bookNames.join(', ')}\n`;
  } catch (err) {
