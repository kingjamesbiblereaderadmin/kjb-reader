...
// ── Fetch (network-first for JS chunks, cache-first for assets) ──────────────
self.addEventListener('fetch', (event) => {
  ...
  // Skip caching for Vite chunks, node_modules, and dev tools
  // These should always be fetched fresh to avoid stale React code
  const skipCache = 
  ...
  event.respondWith(
    caches.match(event.request).then(cached => {
      return cached || fetch(event.request).then(response => {
        // Cache successful HTML/CSS/image responses only
        if (response && response.status === 200) {
          const ct = response.headers.get('content-type') || '';
          // Don't cache JavaScript - let Vite handle it
          if (ct.includes('text/html') || ct.includes('css') || ct.includes('image/')) {
