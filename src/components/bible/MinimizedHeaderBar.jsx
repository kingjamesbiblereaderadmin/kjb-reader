...
export default function MinimizedHeaderBar({ fullscreen, toggleFullscreen, setHideHeader }) {
  return createPortal(
    <div className="print:hidden fixed top-0 left-0 right-0 border-b border-border/50 bg-background/75 backdrop-blur-xl z-[110]" style={{ paddingTop: 'env(safe-area-inset-top)', paddingLeft: 'env(safe-area-inset-left)', paddingRight: 'env(safe-area-inset-right)' }}>
      <div className="w-full max-w-[120rem] mx-auto px-2 py-1.5 flex items-center justify-end gap-1">
        <div className="flex items-center gap-1">
          <button
            ...
            className="p-2 rounded-lg bg-card/60 backdrop-blur-md border border-border/50 hover:bg-accent/15 hover:border-accent/30 text-foreground transition-all duration-200 hover:scale-105 active:scale-95 touch-manipulation flex items-center justify-center cursor-pointer"
          >
