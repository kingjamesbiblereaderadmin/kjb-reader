...
export default function DailyVerseImage({ verse, onClick, onToggleNotif, notifEnabled }) {
  ...
  const handleShare = async (e) => {
    ...
    try {
      ...
      // Pad the square card into a 9:16 portrait frame so apps like Instagram
      // Stories (which auto-crop to 9:16) don't cut off the verse/reference.
      ...
      const offsetY = Math.round((frameH - cardCanvas.height) / 2);
      ctx.drawImage(cardCanvas, 0, offsetY);
  ...
  return (
    <div ref={verseRef} onClick={(e) => { if (!uploadingComplete && !showLightbox) onClick(e); }} className={`w-full ${gradientClass} rounded-2xl shadow-lg px-8 pt-5 pb-8 text-center text-white relative ${uploadingComplete ? 'cursor-default' : 'cursor-pointer'}`} style={bgStyle} onTouchEnd={(e) => { if (!uploadingComplete && !showLightbox) onClick(e); }}>
      ...
      {/* Date display */}
      <div className="absolute bottom-3 left-3 bg-black/30 backdrop-blur-sm rounded-full" style={{ paddingLeft: '14px', paddingRight: '14px', paddingTop: '7px', paddingBottom: '7px' }}>
        <span className="font-medium whitespace-nowrap" style={{ color: '#ffffff', fontFamily: "'Inter', system-ui, sans-serif", fontSize: '12px', lineHeight: '1', display: 'block' }}>
