...
export default function DailyVerseImage({ verse, onClick, onToggleNotif, notifEnabled }) {
  ...
  return (
    <div ref={verseRef} onClick={(e) => { if (!uploadingComplete && !showLightbox) onClick(e); }} className={`w-full ${gradientClass} rounded-2xl shadow-lg px-8 pt-8 pb-20 text-center text-white relative flex flex-col ${uploadingComplete ? 'cursor-default' : 'cursor-pointer'}`} style={bgStyle} onTouchEnd={(e) => { if (!uploadingComplete && !showLightbox) onClick(e); }}>
      ...
      {showVersePanel ? (
        <div className="bg-white/25 backdrop-blur-sm rounded-xl px-6 py-4 shadow-none text-center flex-1 flex flex-col justify-center w-full max-w-full overflow-hidden">
          <p 
