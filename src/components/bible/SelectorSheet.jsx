  ...
  return (
    <Drawer.Root open={open} onOpenChange={(o) => { if (!o) onClose(); }} shouldScaleBackground>
      <Drawer.Portal>
        ...
        <Drawer.Content className="fixed bottom-0 left-2 right-2 sm:left-4 sm:right-4 z-50 flex flex-col rounded-t-2xl bg-card border-t border-border max-h-[85dvh]">
          {/* Drag handle */}
          ...
          <div className="overflow-y-auto flex-1 pb-safe px-2">
            {children}
          </div>
        </Drawer.Content>
      </Drawer.Portal>
