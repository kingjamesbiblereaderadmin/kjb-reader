    ...
    <div className={`max-w-5xl mx-auto px-5 sm:px-8 py-3 ${hideHeader ? 'pt-16' : ''}`}>
      ...
      {!isViewingTitlePage && (
        <div className="text-center mb-6 pt-8" style={{ fontSize: `${zoomLevel / 100}rem` }}>
          <h1 className={`${fontFamily === 'cursive' ? 'cursive-em-style' : 'font-serif'} font-bold text-foreground mb-3 leading-tight ${fontFamily === 'cursive' ? 'text-5xl md:text-6xl' : 'text-3xl md:text-4xl'}`} style={{ fontStyle: 'normal', fontWeight: fontFamily === 'cursive' ? '900' : '700' }}>{book.name}</h1>
          <p className={`font-sans text-xs text-muted-foreground tracking-widest uppercase mt-2 ${fontFamily === 'cursive' ? 'cursive-em-style' : ''}`} style={{ fontStyle: 'normal', fontSize: fontFamily === 'cursive' ? `${zoomLevel / 100 * 0.75}rem` : undefined }}>
            Chapter {pos.chapter}
          </p>
