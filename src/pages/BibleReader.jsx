    ...
    <div className={`max-w-5xl mx-auto px-5 sm:px-8 py-3 ${hideHeader ? 'pt-16' : ''}`}>
      ...
      {!isViewingTitlePage && (
        <div className="text-center mb-6 pt-4" style={{ fontSize: `${zoomLevel / 100}rem` }}>
          <h1 className={`${fontFamily === 'cursive' ? 'cursive-em-style' : 'font-serif'} font-bold text-foreground mb-1 leading-tight ${fontFamily === 'cursive' ? 'text-4xl md:text-5xl' : 'text-3xl md:text-4xl'}`} style={{ fontStyle: 'normal', fontWeight: fontFamily === 'cursive' ? '900' : '700' }}>{book.name}</h1>
